...
function send(ctrl, context) {
  ...
  ws.emit("ctrl", {
    data: JSON.stringify({ cmd: ctrl, context: context, game: "abe" }),
  });
}
  ...
  for (let i = 0; i < maxPredictions; i++) {
    ...
    if (prediction[i].probability.toFixed(2) > 0.9) {
      ...
      if (newState != oldState) {
        if (oldState == "Neutral") {
          let command = newState.includes("StartSelect") ? "START" : newState;
          ...
          let command = newState.includes("StartSelect") ? "START" : oldState;
          ...
          if (newState != "Neutral") {
            command = newState.includes("StartSelect") ? "START" : newState;
            send(command.toUpperCase(), "start");
            console.log("START " + command.toUpperCase());
          }
        }
        //update state
        oldState = newState.includes("StartSelect") ? "START" : newState;
      }
    }
  }
